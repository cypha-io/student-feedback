...
export default function StudentFeedback() {
  ...
  // Parse questions from database into sections
  const parseQuestions = (dbQuestions: any[]) => {
    const sections: any = {};
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const sortedQuestions = response.documents.sort((a: any, b: any) => (a.order || 0) - (b.order || 0));
        setQuestions(sortedQuestions);
        
        // Parse questions into sections and update FEEDBACK_SECTIONS
        const parsedSections = parseQuestions(sortedQuestions);
        if (Object.keys(parsedSections).length > 0) {
          // Update the sections with database questions
          Object.assign(FEEDBACK_SECTIONS, parsedSections);
        }
        
        setQuestionsLoaded(true);
        console.log('Loaded questions from database:', sortedQuestions.length);
        console.log('Parsed sections:', Object.keys(parsedSections));
      } catch (error) {
        console.error('Error fetching questions:', error);
        // Fall back to hardcoded questions if database fails
        setQuestionsLoaded(true);
      }
    };
