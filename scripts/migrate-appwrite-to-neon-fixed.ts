...
// ID mapping to convert Appwrite IDs to UUIDs
const idMappings = {
...
function mapToUUID(originalId: string, type: keyof typeof idMappings): string {
  if (!idMappings[type].has(originalId)) {
    idMappings[type].set(originalId, generateUUID());
  }
  return idMappings[type].get(originalId)!;
}
...
function transformDepartmentData(appwriteDept: AppwriteDocument, index: number) {
  const newId = mapToUUID(appwriteDept.$id, 'departments');
  return {
    id: newId,
    name: String(appwriteDept.name || `Department ${index + 1}`),
    code: String(appwriteDept.code || `DEPT${index + 1}`),
    head: String(appwriteDept.head || 'TBD'),
...
function transformSubjectData(appwriteSubject: AppwriteDocument) {
  const newId = mapToUUID(appwriteSubject.$id, 'subjects');
  return {
    id: newId,
...
function transformClassData(appwriteClass: AppwriteDocument) {
  const newId = mapToUUID(appwriteClass.$id, 'classes');
  return {
    id: newId,
...
function transformTeacherData(appwriteTeacher: AppwriteDocument) {
  const newId = mapToUUID(appwriteTeacher.$id, 'teachers');
  return {
    id: newId,
    name: String(appwriteTeacher.name || ''),
    department: String(appwriteTeacher.department || ''),
    employeeId: String(appwriteTeacher.employeeId || newId),
    class: String(appwriteTeacher.class || ''),
    email: String(appwriteTeacher.email || ''),
    phone: String(appwriteTeacher.phone || ''),
    subjects: Array.isArray(appwriteTeacher.subjects) ? appwriteTeacher.subjects.map(String) : [String(appwriteTeacher.subject || '')],
...
function transformQuestionData(appwriteQuestion: AppwriteDocument) {
  const newId = mapToUUID(appwriteQuestion.$id, 'questions');
  return {
    id: newId,
...
function transformFeedbackData(appwriteFeedback: AppwriteDocument) {
  const newId = mapToUUID(appwriteFeedback.$id, 'feedbacks');
  
  // Map IDs to UUIDs
  const studentId = idMappings.students.get(String(appwriteFeedback.studentId)) || generateUUID();
  const teacherId = idMappings.teachers.get(String(appwriteFeedback.teacherId)) || generateUUID();
  const subjectId = idMappings.subjects.get(String(appwriteFeedback.subjectId)) || generateUUID();
  const classId = idMappings.classes.get(String(appwriteFeedback.classId)) || generateUUID();
  ...
  return {
    ...
    rating: Number(appwriteFeedback.rating) || null,
...
function transformResponseData(appwriteResponse: AppwriteDocument) {
  // Map IDs to UUIDs
  const feedbackId = idMappings.feedbacks.get(String(appwriteResponse.feedbackId)) || generateUUID();
  const questionId = idMappings.questions.get(String(appwriteResponse.questionId)) || generateUUID();
  const teacherId = appwriteResponse.teacherId ? (idMappings.teachers.get(String(appwriteResponse.teacherId)) || generateUUID()) : null;
...
async function clearExistingData() {
  ...
  try {
    ...
    console.log('ℹ️ No existing data to clear or error clearing:', error);
  }
}

async function migrateDataWithMapping() {
  console.log('🚀 Starting Appwrite to Neon migration with ID mapping...');
  ...
  try {
    ...
    if (appwriteDepartments.length > 0) {
      for (let i = 0; i < appwriteDepartments.length; i++) {
        const dept = appwriteDepartments[i];
        const transformed = transformDepartmentData(dept, i);
  ...
  console.log('- IDs have been converted to proper UUIDs');
